...
export const CloneButton: FC<CloneButtonProps> = ({ sourceFolderPath, outputFolderPath, fileExtensions }) => {
  ...
  const handleClone = async () => {
    ...
    (document.getElementById('file-clone-modal') as HTMLDialogElement).showModal();
    try {
      await cloneDir({ sourceFolderPath, outputFolderPath, extToClone: new Set(fileExtensions), onCloneCallback: () => setNumCloned(prev => prev + 1) });
    } catch (error) {
  ...
  return (
    <>
      ...
      <dialog id="file-clone-modal" className="modal">
...
const cloneDir = async ({ sourceFolderPath, outputFolderPath, extToClone, onCloneCallback }: { sourceFolderPath: string; outputFolderPath: string; extToClone?: Set<string>; onCloneCallback?: () => void }) => {
  ...
  const filePromises = files.map(async file => {
    if (file.name === '.DS_Store') {
      ...
      await cloneDir({ sourceFolderPath: await join(sourceFolderPath, file.name), outputFolderPath: await join(outputFolderPath, file.name), extToClone, onCloneCallback });
    } else {
